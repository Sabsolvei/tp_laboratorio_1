...
void controller_mock(ArrayList* pListMaterias)
{
    Materias* m0=NULL;
    m0 = materias_new();
    materias_append (m0, "Programacion I","TP", 116, 28, 0, 1);
    Materias* m1=NULL;
    m1 = materias_new();
    materias_append (m1, "Laboratorio I","TP", 116, 28, 0, 1);
    Materias* m2=NULL;
    m2 = materias_new();
    materias_append (m2, "Matematica I","TP", 87, 28, 0, 1);
    Materias* m3=NULL;
    m3 = materias_new();
    materias_append (m3, "Ingles I","TP", 28, 14, 0, 1);
    Materias* m4=NULL;
    m4 = materias_new();
    materias_append (m4, "SPD","TP", 46, 14, 0, 1);

    Materias* m5=NULL;
    m5 = materias_new();
    materias_append (m5, "Estadistica","TP", 87, 28, 1, 2);
    Materias* m6=NULL;
    m6 = materias_new();
    materias_append (m6, "Metodologia","TP", 28, 14, 1, 2);
    Materias* m7=NULL;
    m7 = materias_new();
    materias_append (m7, "Laboratorio II","TP", 116, 28, 2, 2);
    Materias* m8=NULL;
    m8 = materias_new();
    materias_append (m8, "Arquitectura","TP", 116, 28, 1, 2);
    Materias* m9=NULL;
    m9 = materias_new();
    materias_append (m9, "Ingles II","TP", 28, 14, 1, 2);
    Materias* m10=NULL;
    m10 = materias_new();
    materias_append (m10, "Programacion II","TP", 116, 28, 2, 2);

    Materias* m11=NULL;
    m11 = materias_new();
    materias_append (m11, "Investigacion","TP", 87, 28, 1, 3);
    Materias* m12=NULL;
    m12 = materias_new();
    materias_append (m12, "Org Contable","TP", 28, 14, 1, 3);
    Materias* m13=NULL;
    m13 = materias_new();
    materias_append (m13, "Laboratorio III","TP", 116, 28, 2, 3);
    Materias* m14=NULL;
    m14 = materias_new();
    materias_append (m14, "Org Empresarial","TP", 116, 28, 1, 3);
    Materias* m15=NULL;
    m15 = materias_new();
    materias_append (m15, "Programacion III","TP", 116, 28, 2, 3);
...
int controller_altaMateria(ArrayList* pListMaterias)
{
    ...
    Materias* auxMateria = NULL;
    auxMateria = materias_new();
    retorno = controller_getDataMateria(auxMateria);
    if(retorno == 0)
    {
        id = controller_getNextId(pListMaterias);
...
int controller_getDataMateria(Materias* this)
{
    int retorno = -1, classes = 0, hours = 0, correlativas, cuatrimestre, len;
    ...
    if(retorno == 0)
    {
        ...
        if(retorno == 0)
        {
            ...
            if(retorno == 0)
            {
                ...
                if(retorno == 0)
                {
                    retorno = getValidInt("\nCantidad de correlativas: ","\nRango valido 0 - 10", &correlativas,0,10,3);
                    if(retorno == 0)
                    {
                        retorno = getValidInt("\nCuatrimestre: ","\nRango valido 1 - 6", &cuatrimestre,1,6,3);
                        if(retorno == 0)
                        {
                            retorno = materias_append (this, name, career, hours, classes, correlativas, cuatrimestre);
                            if(retorno != 0)
                            {
...
void controller_printAll(ArrayList* this)
{
    ...
    for(i=0; i<this->len(this); i++)
    {
        ...
        printf("\n\nId: %-3d| Nombre: %-15s| Carrera: %-4s| \nClases: %-3d| Horas: %-3d| Correlativas: %-3d| Nro cuatrimestre: %-3d \n", auxMateria->id, auxMateria->name, auxMateria->career, auxMateria->classes, auxMateria->hours,  auxMateria->correlativas, auxMateria->cuatrimestre);
    }
}
